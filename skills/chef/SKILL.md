...
### Multiple Choice
```typescript
const result = await chef.choice("What's your preferred stack?", [
  "React + Node",
  "Vue + Python",
  "Svelte + Go"
]);
...
// [A] [B] [C]  <- grid buttons
//
// Returns: number (0, 1, 2) if button/letter selected
// Returns: string if user types custom answer
```
...
```typescript
const result = await chef.choice("Framework?", ["React", "Vue", "Svelte"]);
if (typeof result === "number") {
  console.log("Selected option:", ["React", "Vue", "Svelte"][result]);
} else {
  console.log("Custom answer:", result);
}
```
...
- Choice: lists options A-Z, shows grid buttons. Returns index if button/letter selected, or string if user types custom text
